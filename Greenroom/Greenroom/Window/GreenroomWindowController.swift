...
final class GreenroomWindowController: NSWindowController {
    ...
    /// Looks for a "sidebar" folder inside the main app bundle containing index.html.
    private func locateSidebarURLInBundle() -> URL? {
        guard let bundleResourceURL = Bundle.main.resourceURL else {
            return nil
        }

        let candidateURL = bundleResourceURL
    ...
    private func locateSidebarURLRelativeToSourceFile() -> URL? {
        ...
        // …/Greenroom/Greenroom/Window/GreenroomWindowController.swift
        // We need to walk up 3 levels to reach the repo root:
        // Window/ → Greenroom (inner) → Greenroom (outer) → repo root
        let sourceFileURL = URL(fileURLWithPath: #file)

        let repoRootURL = sourceFileURL
            .deletingLastPathComponent() // Window/
            .deletingLastPathComponent() // Greenroom (inner)
            .deletingLastPathComponent() // Greenroom (outer)
